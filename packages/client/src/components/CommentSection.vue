<template>
  <section class="bg-(--bg-card)/70 border border-(--border-subtle)/70 rounded-2xl p-5 flex flex-col gap-4">
    <!-- Header -->
    <div class="flex items-center justify-between">
      <h3 class="text-sm font-black text-(--text-primary) flex items-center gap-2">
        ...
        {{ t('comment.title') }}
        <span v-if="total" class="text-[10px] font-mono text-(--text-faint)">{{ total }}</span>
      </h3>
    </div>

    <!-- 输入区：未登录显示登录引导 -->
    <div v-if="!currentUser" class="flex items-center justify-between gap-3 bg-(--bg-page)/60 border border-dashed border-(--border-subtle)/70 rounded-xl px-4 py-3.5">
      <span class="text-xs text-(--text-faint)">{{ t('comment.loginPrompt') }}</span>
      <button
        @click="openAuth"
    ...
    <div v-else class="flex flex-col gap-2.5">
      <textarea
        ...
        class="w-full bg-(--bg-page)/70 border border-(--border-subtle) rounded-xl px-3.5 py-2.5 text-sm text-(--text-primary) placeholder-zinc-600 resize-none focus:outline-none focus:border-yellow-500/60 focus:ring-1 focus:ring-yellow-500/30 transition-all"
      />
      <div class="flex items-center justify-between">
        <span class="text-[10px] text-(--text-faint) font-mono">{{ draft.length }}/500</span>
        <button
          @click="submitComment"
    ...
    <div v-if="loading" class="flex flex-col gap-3">
      <div v-for="n in 3" :key="n" class="flex gap-3 animate-pulse">
        <div class="w-8 h-8 rounded-full bg-(--bg-hover) shrink-0"></div>
        <div class="flex-1 flex flex-col gap-2">
          <div class="h-3 bg-(--bg-hover) rounded w-1/3"></div>
          <div class="h-3 bg-(--bg-hover)/60 rounded w-2/3"></div>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="items.length" class="flex flex-col divide-y divide-zinc-800/60">
      <div v-for="c in items" :key="c.id" class="py-3.5 flex gap-3">
        <img
          ...
          class="w-8 h-8 rounded-full object-cover border border-(--border-subtle) shrink-0"
          alt=""
        />
        <div class="min-w-0 flex-1">
          <div class="flex items-center gap-2 flex-wrap">
            <span class="text-xs font-bold text-zinc-200">{{ c.nickname || t('comment.anonymous') }}</span>
            <span class="text-[10px] text-(--text-faint)">{{ formatTime(c.createdAt) }}</span>
            <button
              v-if="currentUser && currentUser.id === c.userId"
              @click="removeComment(c)"
              class="ml-auto text-[10px] text-(--text-faint) hover:text-red-400 transition-colors"
            >
              {{ t('comment.delete') }}
            </button>
          </div>
          <p class="text-sm text-(--text-secondary) leading-relaxed mt-1.5 break-words whitespace-pre-line">{{ c.content }}</p>
        </div>
      </div>
      ...
      <button
        v-if="hasMore"
        @click="loadMore"
        class="mt-2 py-2 text-xs text-(--text-faint) hover:text-yellow-400 transition-colors"
      >
        {{ t('comment.loadMore') }}
      </button>
    </div>

    <div v-else class="py-6 text-center text-xs text-(--text-faint)">
      {{ t('comment.empty') }}
    </div>
  </section>
</template>
