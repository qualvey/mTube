<template>
  <div class="w-full h-full overflow-y-auto pt-14 bg-zinc-950">
    <div class="max-w-6xl mx-auto w-full px-4 py-6 flex flex-col gap-6">
      <!-- Top Bar: Back + Title -->
      <div class="flex items-center gap-3">
        <button
          @click="goBack"
          class="shrink-0 w-9 h-9 rounded-full bg-zinc-800/80 hover:bg-zinc-700 border border-zinc-700/60 flex items-center justify-center text-zinc-200 transition-all"
          :title="t('detail.back')"
        >
        ...
        <div class="min-w-0">
          <div class="text-[10px] text-zinc-500 font-bold tracking-widest uppercase">{{ t('detail.pageTitle') }}</div>
          <div class="text-sm text-zinc-300 font-bold truncate">{{ video?.title || '...' }}</div>
        </div>
      </div>

      <div v-if="loading" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 bg-zinc-900 border border-zinc-800 rounded-2xl aspect-video animate-pulse"></div>
        <div class="flex flex-col gap-3">
          <div v-for="n in 4" :key="n" class="bg-zinc-900 border border-zinc-800 rounded-2xl aspect-video animate-pulse"></div>
        </div>
      </div>
      ...
      <template v-else-if="video">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          ...
          <div class="lg:col-span-2 flex flex-col gap-4 min-w-0">
            ...
            <!-- Meta Section -->
            <div class="bg-zinc-900/70 border border-zinc-800/70 rounded-2xl p-5 flex flex-col gap-4">
              <h1 class="text-xl font-black text-white leading-snug">{{ video.title }}</h1>
              ...
              <div class="flex flex-wrap items-center gap-2 text-xs">
                ...
                <div class="flex flex-col">
                  ...
                  <span class="text-[10px] text-zinc-500">{{ t('detail.published') }} {{ formatDate(video.createdAt) }}</span>
                </div>
              </div>

              <div class="flex flex-wrap items-center gap-4 text-xs text-zinc-400 border-y border-zinc-800/70 py-3">
                <span class="flex items-center gap-1.5" :title="t('card.validViews')">
                  ...
                  <span class="font-mono font-bold text-zinc-300">{{ formatCount(video.validViews || 0) }}</span>
                </span>
              ...
              <div v-if="tags.length" class="flex flex-wrap gap-2">
                <span
                  v-for="tag in tags"
                  :key="tag"
                  class="px-2.5 py-1 bg-zinc-800 text-zinc-300 text-[11px] font-medium rounded-lg border border-zinc-700/50"
                >
                  #{{ tag }}
                </span>
              </div>

              <div v-if="video.description" class="text-sm text-zinc-400 leading-relaxed whitespace-pre-line">
                {{ video.description }}
              </div>
            </div>
          ...
          <aside class="flex flex-col gap-3 min-w-0">
            <div class="text-xs font-black text-zinc-500 tracking-widest uppercase px-1">{{ t('detail.related') }}</div>
            ...
            <button
              ...
              class="flex gap-3 bg-zinc-900/60 hover:bg-zinc-800/70 border border-zinc-800/60 rounded-xl p-2.5 text-left transition-all group"
            >
              <div class="relative w-32 shrink-0 aspect-video rounded-lg overflow-hidden bg-zinc-800">
                <img
                  v-if="item.poster"
                  :src="item.poster"
                  :alt="item.title"
              ...
              <div class="min-w-0 flex flex-col justify-center gap-1">
                ...
                <div class="text-[10px] text-zinc-500">{{ item.author }}</div>
            ...
            <div v-if="!relatedLoading && !relatedVideos.length" class="text-xs text-zinc-600 px-1">
              {{ t('detail.noRelated') }}
            </div>
          </aside>
        </div>
      ...
      <div v-else class="py-20 text-center text-zinc-500 text-sm">
        {{ t('detail.notFound') }}
        <button @click="goBack" class="block mx-auto mt-4 px-5 py-2 bg-zinc-800 hover:bg-zinc-700 text-zinc-200 text-xs font-bold rounded-xl border border-zinc-700 transition-all">
          {{ t('detail.back') }}
        </button>
      </div>
    </div>
